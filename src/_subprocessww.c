...
convert_to_unicode(PyObject *arg, void *_param)
{
   ...
   if (arg == Py_None) {
       *param = NULL;
       ...
       arg = PyUnicode_FromEncodedObject(arg,
                                 Py_FileSystemDefaultEncoding,
                                 "strict");
       if (arg == NULL)
           return 0;
   }
   *param = _wcsdup(PyUnicode_AS_UNICODE(arg));
   Py_DECREF(arg);
   if (*param == NULL)
       PyErr_NoMemory();
   return *param != NULL;
...
sp_CreateProcess(PyObject* self, PyObject* args)
{
    ...
    if (! PyArg_ParseTuple(args, "O&O&OOiiOO&O:CreateProcess",
                           convert_to_unicode, &application_name,
                           convert_to_unicode, &command_line,
                           &process_attributes,
                           &thread_attributes,
                           &inherit_handles,
                           &creation_flags,
                           &env_mapping,
                           convert_to_unicode, &current_directory,
                           &startup_info))
        goto error;
    ...
    if (result)
        value = Py_BuildValue("NNii",
                     sp_handle_new(pi.hProcess),
                     sp_handle_new(pi.hThread),
                     pi.dwProcessId,
                     pi.dwThreadId);
    else
        value = PyErr_SetFromWindowsErr(GetLastError());
error:
    safe_free(application_name);
    safe_free(command_line);
    safe_free(current_directory);
...
PyMODINIT_FUNC init_subprocessww(void)
{
    PyObject *d;
    PyObject *m;
    ...
    m = Py_InitModule("_subprocessww", sp_functions);
    if (m == NULL)
        return;
