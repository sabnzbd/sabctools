...
static void encoder_sse_lut() {
    ...
    for (int i = 0; i < 256; i++) {
        ...
        for (; p < 8; p++)
            res[8 + p] = 8 + p +
                         0x40; // +0x40 is an arbitrary value to make debugging slightly easier?  the top bit cannot be set
        ...
        lookups->eolLastChar[i] = ((i == 214 + '\t' || i == 214 + ' ' || i == 214 + '\0' || i == 214 + '\n' ||
                                    i == 214 + '\r' || i == '=' - 42) ? (((i + 42 + 64) & 0xff) << 8) + 0x0a0d003d :
                                   ((i + 42) & 0xff) + 0x0a0d00);
...
template<enum YEncDecIsaLevel use_isa>
static HEDLEY_ALWAYS_INLINE uintptr_t
sse2_expand_store_vector(__m128i data, unsigned int mask, unsigned maskP1, unsigned maskP2, uint8_t *p,
                         unsigned int &shufLenP1, unsigned int &shufLenP2) {
...
do_encode_sse(int line_size, int *colOffset, const uint8_t *HEDLEY_RESTRICT srcEnd, uint8_t *HEDLEY_RESTRICT &dest,
              size_t &len) {
...
# endif
            ...
            if (LIKELIHOOD(0.3, col >= 0)) {
                ...
                *(uint32_t *) p = eolChar;
                ...
                if (HEDLEY_UNLIKELY(i >=
                                    0)) { // this isn't really a proper check - it's only needed to support short lines; basically, if the line is too short, `i` never gets checked, so we need one somewhere
                    i++;
                    break;
                }
